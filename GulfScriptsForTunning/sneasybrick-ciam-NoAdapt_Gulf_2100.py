...
def process_scenario(scenario_name):
   ...
   # Clear the lists at the beginning of each scenario
    ...
    df = pd.read_csv(f"Data/NoAdaptCost_Gulf_2100_Sneasy_{scenario_name}.csv") 
    ...
    result_df_sorted.to_csv(f"logs/ParaTunningFiles/NoAdapt2100/ParaTunning_NoAdapt_Gulf_2100_{scenario_name}.csv", index=False)

    # Getting the best parameters
    best_params = CV5_reg.best_params_
    print(f"Best Parameters for NoAdaptCost Gulf 2100 SSP_{scenario_name}:", best_params)
    ...
    print(f"NoAdaptCost Gulf 2100 MSE for Random Forest on CV train data for ssp {scenario_name}: ", "{:.10f}".format(float(mean_squared_error(y_train, pred))))
    print(f"NoAdaptCost Gulf 2100 MSE for Random Forest on test data for ssp {scenario_name}: ", "{:.10f}".format(float(mean_squared_error(y_test, pred_test))))
    ...
    plt.title(f"FeatureImp NoAdaptCost Gulf 2100 {scenario_name}")
    plt.xlabel("Importance score")
    plt.savefig(f'logs/FeatureImpPlots/NoAdapt2100/NoAdaptCost_Gulf_2100_{scenario_name}.png')
    plt.close()
    ...
    plt.ylabel('MSE')
    plt.legend()
    plt.title(f"NoAdaptCost Gulf 2100 {scenario_name} n estimator tuning")
    plt.savefig(f'logs/ParaTunningPlots/Linemaps/NoAdapt2100/SSPRCP{scenario_name}/NoAdaptCost_Gulf_2100_{scenario_name}_n_estimator_tuning.png')
    plt.close()
    ...
    plt.ylabel('MSE')
    plt.legend()
    plt.title(f"NoAdaptCost Gulf 2100 {scenario_name} max depth tuning")
    plt.savefig(f'logs/ParaTunningPlots/Linemaps/NoAdapt2100/SSPRCP{scenario_name}/NoAdaptCost_Gulf_2100_{scenario_name}_max_depth_tuning.png')
    plt.close()
    ...
    plt.ylabel('MSE')
    plt.legend()
    plt.title(f"NoAdaptCost Gulf 2100 {scenario_name} min samples split tuning")
    plt.savefig(f'logs/ParaTunningPlots/Linemaps/NoAdapt2100/SSPRCP{scenario_name}/NoAdaptCost_Gulf_2100_{scenario_name}_min_samples_split_tuning.png')
    plt.close()
