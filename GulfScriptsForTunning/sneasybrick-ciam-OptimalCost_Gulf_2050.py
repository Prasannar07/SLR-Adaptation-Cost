...
def process_scenario(scenario_name):
   ...
   # Clear the lists at the beginning of each scenario
    ...
    result_df_sorted.to_csv(f"logs/ParaTunningFiles/OptimalCost2050/ParaTunning_OptimalCost_Gulf_2050_{scenario_name}.csv", index=False)

    # Getting the best parameters
    best_params = CV5_reg.best_params_
    print(f"Best Parameters for OptimalCost Gulf 2050 SSP_{scenario_name}:", best_params)
    ...
    print(f"OptimalCost Gulf 2050 MSE for Random Forest on CV train data for ssp {scenario_name}: ", "{:.10f}".format(float(mean_squared_error(y_train, pred))))
    print(f"OptimalCost Gulf 2050 MSE for Random Forest on test data for ssp {scenario_name}: ", "{:.10f}".format(float(mean_squared_error(y_test, pred_test))))
    ...
    plt.ylabel('MSE')
    plt.legend()
    plt.title(f"OptimalCost Gulf 2050 {scenario_name} n estimator tuning")

    plt.savefig(f'logs/ParaTunningPlots/Linemaps/OptimalCost2050/SSPRCP{scenario_name}/OptimalCost_Gulf_2050_{scenario_name}_n_estimator_tuning.png')
    plt.close()
    ...
    plt.ylabel('MSE')
    plt.legend()
    plt.title(f"OptimalCost Gulf 2050 {scenario_name} max depth tuning")

    plt.savefig(f'logs/ParaTunningPlots/Linemaps/OptimalCost2050/SSPRCP{scenario_name}/OptimalCost_Gulf_2050_{scenario_name}_max_depth_tuning.png')
    plt.close()
    ...
    plt.ylabel('MSE')
    plt.legend()
    plt.title(f"OptimalCost Gulf 2050 {scenario_name} min samples split tuning")

    plt.savefig(f'logs/ParaTunningPlots/Linemaps/OptimalCost2050/SSPRCP{scenario_name}/OptimalCost_Gulf_2050_{scenario_name}_min_samples_split_tuning.png')
    plt.close()
