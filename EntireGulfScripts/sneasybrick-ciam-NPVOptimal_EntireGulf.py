...
def process_scenario(scenario_name, para_grid):
    ...
    for row_index, row in df.iterrows():
        ...
        Y = Y.rename(columns={Y.columns[0]: 'AverageNPCOptimalCost'}) 
    ...
    plt.title(f"FeatureImp NPVOptimal EntireGulf {scenario_name}")
    plt.xlabel("Segments")
    plt.ylabel("Feature Importance")
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(f"logs/FeatureImpEntireGulfStackBarPlots/NPVOptimal/FeatureImp_NPVOptimal_EntireGulf_{scenario_name}.png")
    ...
    save_path = f"logs/FeatureImpEntireGulfFiles/NPVOptimal/FeatureImp_NPVOptimal_EntireGulf_{scenario_name}.csv"
